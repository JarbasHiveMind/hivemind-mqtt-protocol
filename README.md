...
```
<prefix>/<name>/c2s/<satellite_id>     # satellite → hub  (hub subscribes …/c2s/+)
<prefix>/<name>/s2c/<satellite_id>     # hub → satellite
...
| `name` | node identity name | Hub identifier in topics |
